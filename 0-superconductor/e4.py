...
# 计算电势
phi =p*Y* (1/((X-d)**2+Y**2)**1.5-(a**2/d**2)*(1/((X-b)**2+Y**2)**1.5))
phi[np.isnan(phi)] = 0
# 计算电场（电势的梯度）
...
plt.quiver(d, 0, 0, 1.5, angles='xy', scale_units='xy',color='darkorange', scale=1,zorder=3,pivot ='middle')
plt.quiver(b, 0, 0, -1,angles='xy', scale_units='xy', zorder=3,pivot = 'middle',scale=1, color='darkorange')
# 添加电偶极子矢量箭头
dipole_length2 = 1  # 电偶极子矢量箭头的长度


plt.text(b-0.5, 0, "p\'", color='darkorange', ha='center', va='center',zorder=5) 
plt.text(d+0.5, 0, "p", color='darkorange', ha='center', va='center',zorder=5) 
