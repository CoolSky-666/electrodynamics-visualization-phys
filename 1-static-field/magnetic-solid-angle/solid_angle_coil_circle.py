...
def make_circle(radius: float, n: int) -> np.ndarray:
...
def analytic_Bz_center_circle(mu0: float, current: float, radius: float, z: float) -> float:
...
def plot_figure(
    ...
    Bz_ana = analytic_Bz_center_circle(mu0, current, radius, z0)
    axes[1].text(
        0.0,
        extent[3] * 0.88,
...
def main() -> None:
    ...
    parser.add_argument("--coil-n", type=int, default=360)
    ...
    vertices = make_circle(args.radius, n=args.coil_n)
    ...
    out_path = Path(args.out) if args.out is not None else Path("solid_angle_coil_circle.png")
